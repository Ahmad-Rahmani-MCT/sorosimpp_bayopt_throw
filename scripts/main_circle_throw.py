...
# defining the smoothstep linspace function to generate an array of smoothstep interpolated values 
...
def simulate_sys_runup(amplitude: int, frequency: int, runup_steps: int, release_step: int, input_scaler: MinMaxScaler, state_scaler: MinMaxScaler) : 
        ...
        #u_array = np.vstack((u_smoothstep_runup, u_unmodulated[len(smoothstep_factor_runup):,:])) 
...
circle_throw_performance_dirname = "circle_throw_performance" 
circle_throw_performance_path = os.path.join(script_directory, circle_throw_performance_dirname)
os.makedirs(circle_throw_performance_path, exist_ok=True) 

MIL_result_dirname = f"MIL_results_{des_land_pos[0]}_{des_land_pos[1]}" 
MIL_result_path = os.path.join(circle_throw_performance_path, MIL_result_dirname)
...
MIL_trajectory_filename = "circle_MIL_sim_logs.csv" 
...
sim_result_path = os.path.join(circle_throw_performance_path, sim_result_dirname)
