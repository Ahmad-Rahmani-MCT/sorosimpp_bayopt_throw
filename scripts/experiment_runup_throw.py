...
def simulate_sys_runup(u_step_runup: np.array, ramp_steps_runup: int, u_step: np.array, ramp_steps: int, release_step: int, input_scaler: MinMaxScaler, state_scaler: MinMaxScaler, des_land_pos: list): 
...
def main():
    ...
    print(f"Starting Master Runup Experiment Loop for {len(targets)} targets.")
    ...
    for i, target in enumerate(targets):
        ...
        def objective(trial) : 
            u1_step_runup = trial.suggest_float("act1_step_runup", 0, umax)  
            u2_step_runup = trial.suggest_float("act2_step_runup", 0, umax) 
            u3_step_runup = trial.suggest_float("act3_step_runup", 0, umax)  
            ...
            _, _, _, _, _, dist, runup_traj = simulate_sys_runup(u_step_runup=u_step_runup, ramp_steps_runup=ramp_steps_runup, u_step=u_step, ramp_steps=ramp_steps, release_step=release_step, input_scaler=input_scaler, state_scaler=state_scaler, des_land_pos=des_land_pos)    
            
            r_x = runup_traj[:, ee_x_idx]
            r_y = runup_traj[:, ee_y_idx] 
            t_x, t_y = des_land_pos[0], des_land_pos[1]
            t_norm = np.sqrt(t_x**2 + t_y**2) + 1e-6 
        ...
        u_step_runup = np.array([best_params["act1_step_runup"], best_params["act2_step_runup"], best_params["act3_step_runup"]])
        ramp_steps = best_params["ramp_steps"]
        release_step = best_params["release_step"] 

        # Forward pass to get MIL data
        u_data, x_data, velocities, mil_land_pos, release_idx, mil_dist, _ = simulate_sys_runup(u_step_runup=u_step_runup, ramp_steps_runup=ramp_steps_runup, u_step=u_step, ramp_steps=ramp_steps, release_step=release_step, input_scaler=input_scaler, state_scaler=state_scaler, des_land_pos=des_land_pos) 
        ...
        results_path = os.path.join(script_path, "final_runup_throwing_experiment.csv")
    ...
    print("\n=== RUNUP THROWING EXPERIMENT FULLY COMPLETED ===")
